...
class ImagePickerUtil {
  ...
  static Future<XFile?> takePhotoOrFromLibrary(
      {ImageSourceType imageSource = ImageSourceType.gallery,
        MyRequestType requestType = MyRequestType.image}) async {
    // 图片
  ...
  static Future<List<XFile>?> takeManyPhotoOrFromLibrary(
      {ImageSourceType imageSource = ImageSourceType.gallery,
        ...
        MyRequestType requestType = MyRequestType.image}) async {
    // 图片
    if (imageSource == ImageSourceType.gallery && await PermissionUtils.requestPhotoPickPermission()) {
      List<XFile> xFiles = [];
      List<AssetEntity> models =
      await selectImages(maxCount: maxCount, requestType: requestType);
  ...
  static Future<List<AssetEntity>> selectImages(
      {int maxCount = 1, MyRequestType requestType = MyRequestType.image}) async {
    final List<AssetEntity> pickResult = await AssetPicker.pickAssets(
      currentContext,
      pickerConfig: AssetPickerConfig(
          requestType: requestType == MyRequestType.image ? RequestType.image : RequestType.video,
          maxAssets: maxCount,
          // 最多选择数量
